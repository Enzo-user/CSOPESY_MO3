...
are needed.
  ...
  ---------------------------------------
    ...
    2. Copy ascii_big.txt and config.txt into the project folder (the folder
       ...
       config.txt in between test cases.
    3. Press Run/Debug.
...
or copy them next to the executable. If the font cannot be found, the console prints a
...
is needed.
    ...
    marquee-text   The text the marquee scrolls. Optional double quotes are
                   stripped, so leading and trailing spaces can be kept.
                   Default "Hello, World!" (set_text does the same at run time)
    refresh-rate   Marquee animation refresh in milliseconds (whole number,
                   1 or more). Default 100 (set_speed at run time)
    polling-rate   Keyboard polling interval in milliseconds (whole number,
                   1 to 1000). Default 10 (set_poll at run time)

One "key value" pair per line; everything after a "#" is a comment; blank
lines are ignored; "-" and "_" in a key are interchangeable. A missing file,
an unknown key or an unusable value keeps the built-in default and prints a
note. The settings in force are printed under the welcome header, so a
recording shows which values the run actually used:
