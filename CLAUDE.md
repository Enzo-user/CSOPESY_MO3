...
1. Display a welcome header that includes text for **"Group developer:"** and **"Version date:"**.
...
5. Marquee console (MO3 specification): `start_marquee` starts the marquee "animation", `stop_marquee` stops it, and `set_speed <milliseconds>` sets the animation refresh interval. The marquee scrolls the saved text across the top eight rows of the console.
...
The header lists the actual group members in the sample's `Last name, First name` format and the current version date. Keep the names and the date in sync across `main.cpp`, `README.md`, `README.txt`, and `CONTRIBUTIONS.md`.
...
- The marquee font is `ascii_big.txt` (8 rows per printable character, `!` to `~`), loaded at run time from the working directory or the executable's directory. If it is missing, print a warning and scroll the text as a single plain row.
